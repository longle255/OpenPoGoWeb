    ...
    buildMenu: function (user_id, menu) {
        ...
        switch (menu) {
                ...
                for (var i = 0; i < current_user_bag_items.length; i++) {
                    if (current_user_bag_items[i].inventory_item_data.item.count > 0) {
                        out += '<div class="col s12 m6 l3 center" style="float: left"><img src="image/items/' +
                            ...
                            '</b><br><button class="chip btn-discard-item" data-account=' + self.settings.users[user_id] +
                            ' data-itemId=' + current_user_bag_items[i].inventory_item_data.item.item_id + '>discard</button> ' +
                            '</div>';
            ...
            case 4:
                ...
                break;
            case 5:
                $('#subtitle').html('Logs of user' + self.settings.users[user_id]);
                out = '<div class="row" style="max-height: 80vh;overflow-y: scroll;" id="logs-' + self.settings.users[user_id] + '"><div class="col s12">'
                var logs = self.user_data[self.settings.users[user_id]].logs;
                for (var i = 0; i < logs.length; i++) {
                    out += " <div class='log-item'> <span class='log-date'>" + logs[i].event + "&nbsp;&nbsp;&nbsp;</span><span class='" + logs.color + "'>" + logs[i].message + "</span></div>"
                }
                out += '</div></div>'
            ...
            <div class="collapsible-header bot-name">{0}</div>\
                <div class="collapsible-body">\
                    <ul class="bot-items" data-user-id="{1}">\
                       ...
                       <li><a class="bot-' + i + ' waves-effect waves-light btn tLogs">Logs</a></li>\
                   </ul>\
               </div>\
           </li>';
